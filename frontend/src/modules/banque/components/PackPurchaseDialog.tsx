...
export function PackPurchaseDialog({ open, onClose }: PackPurchaseDialogProps) {
  ...
  async function handleBuy() {
    if (!memberUuid || !packDefUuid) return
    await buyPackMutation.mutateAsync({ memberUuid, packDefinitionUuid: packDefUuid })
    onClose()
    setMemberUuid('')
    setPackDefUuid('')
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      ...
      >
        <h2 className="text-lg font-semibold text-slate-900">{t('ops.flights.packPurchase.title', 'Acheter un forfait')}</h2>
        <p className="mt-1 text-sm text-slate-500">{t('ops.flights.packPurchase.description', 'Sélectionnez un membre et un type de forfait.')}</p>

        <div className="mt-4 space-y-4">
          <div className="space-y-1">
            <Label>{t('ops.flights.packPurchase.member', 'Membre')}</Label>
            <SearchableSelect
              options={memberOptionsForSelect}
              value={memberUuid}
              onChange={setMemberUuid}
              placeholder={t('ops.flights.packPurchase.selectMember', 'Sélectionner un membre…')}
            />
          </div>
          <div className="space-y-1">
            <Label>{t('ops.flights.packPurchase.pack', 'Forfait')}</Label>
            <SearchableSelect
              options={packOptions}
              value={packDefUuid}
              onChange={setPackDefUuid}
              placeholder={t('ops.flights.packPurchase.selectPack', 'Sélectionner un forfait…')}
            />
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <Button onClick={handleBuy} disabled={!memberUuid || !packDefUuid || buyPackMutation.isPending}>
            {buyPackMutation.isPending && <Loader2 className="mr-1 h-4 w-4 animate-spin" />}
            {t('ops.flights.packPurchase.buy', 'Acheter')}
          </Button>
        </div>
      </div>
    </div>
