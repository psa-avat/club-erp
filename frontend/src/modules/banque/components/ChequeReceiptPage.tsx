/*
    ...
    - banque: Cheque receipt — records each cheque as a separate Draft entry
      debiting the pending cheques account (5112), ready for a later remittance
...
function emptyRow(): ChequeRow {
  return { account_uuid: '', tiers_uuid: '', description: '', amount: '' }
}

export function ChequeReceiptPage() {
  ...
  const [rows, setRows] = useState<ChequeRow[]>([emptyRow(), emptyRow()])
  ...
  const pendingAccountUuid = typeof rawSettings.pending_account_uuid === 'string' ? rawSettings.pending_account_uuid : ''
  ...
  const settingsMissing = !pendingAccountUuid

  function updateRow(index: number, patch: Partial<ChequeRow>) {
  ...
  async function handleGenerate() {
    ...
    try {
      for (const row of validRows) {
        const amount = decimalOrZero(row.amount).toFixed(2)
        await createEntryMutation.mutateAsync({
          fiscal_year_uuid: activeFiscalYearUuid,
          journal_uuid: bqJournal.uuid,
          entry_date: entryDate,
          description: row.description.trim() || t('cheque.receipt.title'),
          reference: null,
          lines: [
            { account_uuid: pendingAccountUuid, debit: amount, credit: '0', description: row.description.trim() || null, tiers_uuid: null },
            { account_uuid: row.account_uuid, debit: '0', credit: amount, description: row.description.trim() || null, tiers_uuid: row.tiers_uuid || null },
          ],
        })
      }
      setSuccessMessage(t('cheque.receipt.success', { count: validRows.length }))
      setRows([emptyRow(), emptyRow()])
    } catch (error) {
      setLocalError(toErrorMessage(error, t('cheque.receipt.error')))
    } finally {
  ...
  return (
    <section className="space-y-4">
      <div>
        <h2 className="text-lg font-semibold text-foreground">{t('cheque.receipt.title')}</h2>
        <p className="text-sm text-muted-foreground">{t('cheque.receipt.description')}</p>
      </div>
      ...
      {activeFiscalYearUuid && bqJournal && settingsMissing && <Alert>{t('cheque.receipt.settingsMissing')}</Alert>}
      {activeFiscalYearUuid && bqJournal && !settingsMissing && validRows.length === 0 && (
        <Alert>{t('cheque.receipt.noRows')}</Alert>
      )}
      ...
      <div className="space-y-3 rounded-lg border bg-muted/40 p-4">
        <div className="overflow-x-auto overflow-y-visible rounded-lg border bg-card">
          <table className="min-w-full divide-y divide-border text-sm">
            <thead className="bg-muted">
              <tr>
                <th className="sticky left-0 z-10 bg-muted px-3 py-2 text-left font-medium text-muted-foreground">{t('cheque.receipt.columns.account')}</th>
                <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('cheque.receipt.columns.tiers')}</th>
                <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('cheque.receipt.columns.description')}</th>
                <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('cheque.receipt.columns.amount')}</th>
        ...
        <div className="flex justify-end">
          <Button type="button" size="sm" variant="secondary" onClick={addRow}>
            {t('cheque.receipt.addRow')}
          </Button>
        </div>
      </div>

      <Button type="button" disabled={!canSubmit} onClick={() => void handleGenerate()}>
        {isSubmitting ? t('cheque.receipt.generating') : t('cheque.receipt.generate')}
      </Button>
    </section>
  )
}
