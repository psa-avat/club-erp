...
export function RegistrationPanel({ open, onClose, member, year, onCompleted }: Props) {
  ...
  return (
    <Dialog
      ...
      className="ml-auto mr-0 flex h-[100vh] max-h-[100vh] max-w-3xl flex-col overflow-hidden rounded-none"
    >
      <div className="flex h-full flex-col">
        <div className="border-b border-outline-variant px-6 py-4">
          ...
          <p className="text-sm text-on-surface-variant">
            {member ? `${member.first_name} ${member.last_name} • ${member.account_id}` : t('sheet.selectMember')}
          </p>
        </div>
