...
| **3.2** | #2 **VI & HelloAsso** | `/workspace/vi` | Bons VIs → tab `bons`<br/>VI types → tab `types`<br/>VI planning → tab `planning`<br/>HelloAsso achats → tab `achats`<br/>Import VI → tab `import`<br/>Sync VI → tab `sync` | `routes/assets.tsx` (partiel) | Dialog nouveau type VI<br/>Sheet import HelloAsso |
